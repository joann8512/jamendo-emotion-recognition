...
class Processor():
    def __init__(self,
        ...
        self.preprocessor = ml.loading.load_preprocessor(params.get("preprocessor"), self.device)
        dataset_name = params.get("dataset", "name")  # jamendo
        self.input_directory = '/share/hel/datasets/fs/MTG-Jamendo'#os.path.join(data_dir, dataset_name)
        self.output_directory = os.path.join('.', params.get("output_dir_name")) #os.path.join(data_dir, dataset_name,
                                             #params.get("output_dir_name"))
        if not os.path.exists(self.output_directory):
            os.makedirs(self.output_directory)

    def process(self) -> None:
        files = glob(os.path.join(self.input_directory, "audio", "*", "*.mp3")) #glob(os.path.join(self.input_directory, "mp3", "*", "*.mp3"))
